    ...
    return (
      <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '200px' }}>
        <div className="spinner-border text-primary" role="status">
          <span className="visually-hidden">Loading election history...</span>
        </div>
      </div>
    );
  }
        ...
        <Card title="Election History">
                      ...
                      <td>
                        {election.isCurrent ? (
                          <span className={`badge bg-${election.isActive ? 'success' : 'warning'}`}>
                            {election.isActive ? 'Active (Current)' : 'Inactive (Current)'}
                          </span>
                        ) : (
                          <span className="badge bg-secondary">Concluded</span>
                        )}
                      </td>
          ...
          <div className="mt-3 text-center">
            <Button onClick={fetchContractData} variant="outline-secondary" disabled={loading} size="sm">
              {loading ? (
                <><span className="spinner-border spinner-border-sm me-2"></span>Refreshing...</>
              ) : (
                <><i className="bi bi-arrow-clockwise me-2"></i>Refresh History</>
              )}
            </Button>
          </div>
        </Card>
